...
public class AdminDaoImpl extends DigifyJdbcTemplate implements AdminDao {
	...
	public String insertFile(User user, String fileName, String tableName, Object obj, String iconFileName) {
		...
		if (obj != null && obj instanceof HomepageContent) {
			...
			Sqlquery = "INSERT INTO " + tableName
					+ " (user_id , product_name , product_description , product_image , external_link , icon_img, created_on , created_by  ) VALUES "
					+ "(?,?,?,?,?,?,'" + currentTime1 + "',?)";
			rowInsert = getJdbcTemplate().update(Sqlquery, userid, products.getProductName(),
					products.getProductDescription(), fileName, products.getExternalLink(), iconFileName,
					user.getName());
		} else if (obj != null && obj instanceof Services) {
			Services services = null;
			services = (Services) obj;
			Sqlquery = "INSERT INTO " + tableName
					+ " (user_id , service_name , service_description , service_image , external_link , icon_img, created_on , created_by  ) VALUES "
					+ "(?,?,?,?,?,?,'" + currentTime1 + "',?)";
			rowInsert = getJdbcTemplate().update(Sqlquery, userid, services.getServiceName(),
					services.getServiceDescription(), fileName, services.getExternalLink(), iconFileName,
					user.getName());
		}
		...
		if (content != null) {
			...
			if (content instanceof Products) {
				...
				Sqlquery = "UPDATE products SET "
						+ " product_image = ?,external_link = ?,product_description = ? , product_name = ?,icon_img=?,modified_on=now() WHERE id = ?;";
				updateInsert = getJdbcTemplate().update(Sqlquery, fileName, product.getExternalLink(),
						product.getProductDescription(), product.getProductName(), iconFileName, product.getId());
			}
			if (content instanceof Services) {
				Services service = null;
				service = (Services) content;
				Sqlquery = "UPDATE services SET "
						+ " service_image = ?,external_link = ?,service_description = ? , service_name = ?,icon_img=?,modified_on = now() WHERE id = ?;";
				updateInsert = getJdbcTemplate().update(Sqlquery, fileName, service.getExternalLink(),
						service.getServiceDescription(), service.getServiceName(), iconFileName, service.getId());
			}
		}
