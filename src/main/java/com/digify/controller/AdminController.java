...
//			String filepath = extractservice.extracted(file, uploadNews, tableName);
    ...
    public String getAllOrders(ModelMap model) {
		...
		List<UserOrders> userAllOrders =  adminService.getAllOrders();
        model.addAttribute("active", "allorder");
        model.addAttribute("userAllOrders", userAllOrders);
        return "admin/allorders/allOrders";
    }
    
    @RequestMapping(value = {"/getOrderByOrderId/{orderId}"}, method = {RequestMethod.GET})
    public String getOrderByOrderId(ModelMap model , @PathVariable("orderId") String orderId) {
        model = adminService.addUserInModel(model);
        model = adminService.addListHomePageContent(model);

		List<UserOrderResponseDto> listOfOrderDetails =  adminService.getOrderByOrderId(orderId);
        model.addAttribute("active", "allorder");
        model.addAttribute("listOfOrderDetails", listOfOrderDetails);
        model.addAttribute("orderId", orderId);
        return "admin/allorders/listOfOrderDetails";
    }
