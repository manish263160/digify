...
function addTocart(digify, loggedInUserId, productOrServiceId, callFor) {

    console.log("loggedInUserId==" + loggedInUserId + " productOrServiceId::" + productOrServiceId + " callFor::" + callFor);

//	var data= { 'callFor' :callFor};
    $.ajax({
        url: digify + '/ecommerce/addTocart/' + loggedInUserId + "/" + productOrServiceId + "/" + callFor,
        type: 'POST',
        success: function (result) {
            console.log("result==" + result);
            if(result) {
                alert("Your Item has been added in your cart");
                var number = $("#cartNumber").html();
